...
def add_npcs(world, nnpcs, pos_agents, ego_start, pos_inits):
    ...
    for ix in ixes:
        ...
        try:
            ...
            if agentbp.has_attribute('color'):
                agentbp.set_attribute('color', f'{np.random.randint(256)},{np.random.randint(256)},{np.random.randint(256)}')
            agent = world.spawn_actor(agentbp, pos_inits[ix])
...
def init_env(host, port, width, height, timeout, calibf, ncarcalib, rnd_seed,
             map_name, p_assets):
...
def determine_filter(bboxes, cameras, depth_data):
    ...
    for depi, dep in enumerate(depth_data):
        ...
        for boxi,box in enumerate(in_cam_boxes):
            ...
            for boxj,obox in enumerate(in_cam_boxes):
